...
//map.readValue是把json转换成item
public class TwitchClient {
    private static final String TOKEN = "Bearer 6e36qwpkb2vffcfs5pq3l687ez8gaj";
    private static final String CLIENT_ID = "u122bcvz3gzo3pkyotnc2b03hnfahw";
    private static final String TOP_GAME_URL_TEMPLATE = "https://api.twitch.tv/helix/games/top?first=%s"; //%s是占位的
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        //如果没有gamename就是top TOP_GAME_URL_TEMPLATE
        if (gameName.equals("")) {
            ...
            //otherwise gamename DEFAULT_GAME_LIMIT
    ...
    private String buildSearchURL(String url, String gameId, int limit) {
        try {
            gameId = URLEncoder.encode(gameId, "UTF-8"); //为什么要做encoding? convert 特殊字符 如空格
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return String.format(url, gameId, limit); //替换url里面的两个%s的位置
    }
    ...
    private List<Item> getItemList(String data) throws TwitchException {
        ObjectMapper mapper = new ObjectMapper();
        try {
            return Arrays.asList(mapper.readValue(data, Item[].class));  //parse json to item list
        } catch (JsonProcessingException e) {
            e.printStackTrace();
    ...
    private List<Item> searchStreams(String gameId, int limit) throws TwitchException {
        ...
        for (Item item : streams) {
            item.setType(Item.ItemType.STREAM);
            item.setUrl(TWITCH_BASE_URL + item.getBroadcasterName()); //这两个得自己customize
        }
        return streams;
    }
    ...
    private List<Item> searchClips(String gameId, int limit) throws TwitchException {
        ...
        for (Item item : clips) {
            item.setType(Item.ItemType.CLIP);
        }
        return clips;
    }
    ...
    private List<Item> searchVideos(String gameId, int limit) throws TwitchException {
        ...
        for (Item item : videos) {
            item.setType(Item.ItemType.VIDEO);
        }
        return videos;
    }

    public List<Item> searchByType(String gameId, Item.ItemType type, int limit) throws TwitchException {  //就这俩public function
    ...
    public Map<String, List<Item>> searchItems(String gameId) throws TwitchException {  //就这俩public function
