...
import com.example.jupiter.db.MySQLDBUtil;
...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException {
        try {
            //com.mysql.cj.jdbc.Driver driver = new com.mysql.cj.jdbc.Driver(); 的意思 但是下面这个更灵活，因为没有这个library的话可以临时加
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance(); //处理corner case
            conn = DriverManager.getConnection(MySQLDBUtil.getMySQLAddress()); //跟db 连接
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        saveItem(item);
        String sql = "INSERT IGNORE INTO favorite_records (user_id, item_id) VALUES (?, ?)"; //MySQL的写法
        try {
            PreparedStatement statement = conn.prepareStatement(sql);  //PreparedStatement能认识这个sql语句
            statement.setString(1, userId);
            statement.setString(2, item.getId()); //填这两个？的操作， 用string.format的写法的话 不会有pre判断
            statement.executeUpdate(); //执行语句，如果插入重复的userID和itemID,会扔exception,但我们加了IGNORE 就不会扔了
    ...
    public void unsetFavoriteItem(String userId, String itemId) throws MySQLException {
        ...
        String sql = "DELETE FROM favorite_records WHERE user_id = ? AND item_id = ?"; //唯一的区别在这里
        try {
    ...
    public void saveItem(Item item) throws MySQLException { //不用saveUser因为发送指令需要先Login 所以user肯定存在
        if (conn == null) {
            System.err.println("DB connection failed");
            throw new MySQLException("Failed to connect to Database");
        }
        String sql = "INSERT IGNORE INTO items VALUES (?, ?, ?, ?, ?, ?, ?)"; //加IGNORE 就不用管之前是否存在这个item了
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
    ...
    public Set<String> getFavoriteItemIds(String userId) throws MySQLException {  //仅返回id，recommendation的helper function
        if (conn == null) {
            System.err.println("DB connection failed");
            throw new MySQLException("Failed to connect to Database");
        }
        ...
        String sql = "SELECT item_id FROM favorite_records WHERE user_id = ?"; //sql语句
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, userId);
            ResultSet rs = statement.executeQuery(); //因为是get function 所以用的是executeQuery, 这个rs是个iterator，从-1开始
            while (rs.next()) { //这个结合了hasNext()和next()
    ...
    public Map<String, List<Item>> getFavoriteItems(String userId) throws MySQLException { //返回item所有信息
        if (conn == null) {
            System.err.println("DB connection failed");
            throw new MySQLException("Failed to connect to Database");
        }
        ...
        Set<String> favoriteItemIds = getFavoriteItemIds(userId); //直接用我们写好的function
        String sql = "SELECT * FROM items WHERE id = ?"; //*代表返回所有column
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            for (String itemId : favoriteItemIds) {
                ...
                if (rs.next()) {
                    Item.ItemType itemType = Item.ItemType.valueOf(rs.getString("type")); //把string convert成enum
                    Item item = new Item.Builder().id(rs.getString("id")).title(rs.getString("title")) //注意 是用的builder
                    ...
                    itemMap.get(rs.getString("type")).add(item); //一共三个type，video，clip，stream 每个后面跟了一个list
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        try {
            ...
            return statement.executeUpdate() == 1; //存
        } catch (SQLException e) {
            e.printStackTrace();
